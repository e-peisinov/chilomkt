<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Pagina Contacto</h1>
            <p class="text-gray-500 text-sm mt-1">Gestiona el contenido de la pagina de contacto y mensajes</p>
        </div>
    ...
    <div class="space-y-3">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ open: false }">
            <button @click="open = !open" class="w-full flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs text-gray-400">Email, telefono, direccion, WhatsApp</span>
                </div>
                <svg class="w-5 h-5 text-gray-400 transition-transform duration-300" :class="open && 'rotate-180'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </button>
            <div x-show="open" x-collapse class="border-t border-gray-100">
                <div class="p-4">
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">WhatsApp (sin +, solo numeros)</label>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ open: false }">
            ...
            <div x-show="open" x-collapse class="border-t border-gray-100">
                <div class="p-4">
                    @if($mensajes->isEmpty())
                        <p class="text-gray-400 text-sm text-center py-6">No hay mensajes aun.</p>
                    @else
                        <div class="space-y-3">
                                ...
                                <div class="p-4 rounded-lg border {{ $msg->leido ? 'bg-gray-50 border-gray-100' : 'bg-blue-50 border-blue-100' }}">
                                    <div class="flex items-start justify-between">
                                        ...
                                        <div class="flex items-center gap-1 ml-4">
                                            <button wire:click="marcarLeido({{ $msg->id }})" class="text-gray-400 hover:text-chilo-dark p-1" title="{{ $msg->leido ? 'Marcar no leido' : 'Marcar leido' }}">
                                                @if($msg->leido)
